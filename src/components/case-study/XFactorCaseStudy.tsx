...
export default function XFactorCaseStudy({ project }: { project: Project }) {
  return <main id="main-content" tabIndex={-1} className="flex-1 pt-32 md:pt-40"><article className="mx-auto max-w-6xl px-6">
    ...
    <header className="py-14 md:py-20">
      ...
      <dl className="mt-10 grid gap-6 border-t border-edge pt-7 sm:grid-cols-3"><div><dt className="text-xs uppercase tracking-[0.16em] text-ink-faint">Market</dt><dd className="mt-2 text-sm leading-6 text-ink">UAE</dd></div><div><dt className="text-xs uppercase tracking-[0.16em] text-ink-faint">Location</dt><dd className="mt-2 text-sm leading-6 text-ink">Dubai, UAE</dd></div><div><dt className="text-xs uppercase tracking-[0.16em] text-ink-faint">Role</dt><dd className="mt-2 text-sm leading-6 text-ink">{project.role}</dd></div></dl>
    ...
    <section id="pattern" className="border-t border-edge py-16 md:py-24"><Heading eyebrow="Supporting Language">Pattern & Brand Guidelines</Heading><Image src={`${base}/brand-guidelines/x-factor-interior-design-brand-guidelines-supporting-pattern.webp`} alt="X Factor Interior Design supporting pattern shown across light, charcoal, and warm-neutral colorways." width={2880} height={1620} sizes="(min-width: 1152px) 1152px, 100vw" className="mt-10 h-auto w-full" /><p className="mt-6 max-w-3xl leading-8 text-ink-soft">The flowing pattern extends the contrast between structure and sensitivity into backgrounds, stationery, and editorial details without competing with typography or imagery.</p><div className="mt-10"><XFactorBrandGuidelinesShowcase /></div></section>

    <section id="company-profile" className="border-t border-edge py-16 md:py-24"><Heading eyebrow="Corporate Communication">Company Profile</Heading><p className="mt-7 max-w-3xl text-lg leading-8 text-ink-soft">The identity was extended into a structured company profile that balanced editorial refinement with clear communication of the studio&apos;s philosophy, services, process, and capabilities.</p><div className="mt-10"><XFactorCompanyProfileShowcase /></div></section>

    <section id="stationery" className="border-t border-edge py-16 md:py-24"><Heading eyebrow="Applications">Stationery</Heading><p className="mt-7 max-w-3xl text-lg leading-8 text-ink-soft">The identity was translated into a restrained stationery system using the same typographic hierarchy, neutral palette, supporting pattern, and precise spacing.</p><div className="mt-10"><XFactorStationeryShowcase /></div></section>

    <section id="social" className="border-t border-edge py-16 md:py-24"><Heading eyebrow="Content">Social Media System</Heading><p className="mt-7 max-w-3xl text-lg leading-8 text-ink-soft">The social system extends the identity into repeatable content around design philosophy, turnkey service, process, execution, and residential detail—maintaining luxury through restraint rather than excess.</p><XFactorSocialMediaShowcase /></section>
